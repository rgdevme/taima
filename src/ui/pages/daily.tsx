...
import { Entry } from '../../common/types/entries'
...
import { calculateHours, processEntry } from '../../common/utils/entries'
...
import { Button, Divider, Group, Popover, Stack } from '@mantine/core'
...
export const Daily = () => {
  ...
  useEffect(() => {
    tasks.request({
      params: {
        listIds: ctx.lists.map(l => l.id),
        date_updated_gt: pastTercil,
      },
    })
  }, [ctx.lists])
  ...
  return (
    ...
    >
      <div className='detail'>
        <TodayCounter
          max={workHours}
          value={days[todaydate]}
        />
        <DateInput
          type='range'
          value={selectedWeek.map(d => d.toDate())[0]}
          // onChange={djs => {
        ...
        <div className='links'>
          <Button
            leftSection={routes.config.icon}
            onClick={() => {
              navigate(routes.config.path)
            }}
          />
          <Button
            leftSection={<AiOutlineCopy />}
            onClick={() => {
            ...
            <Popover.Dropdown>
              <Stack>
                ...
                <Group>
                  ...
                  <Button onClick={() => ipcRenderer.send('exit-app')} />
                </Group>
              </Stack>
            </Popover.Dropdown>
          </Popover>
